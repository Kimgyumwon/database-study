-- Active: 1758675368804@@127.0.0.1@3306@quiz
-- 1. 자료형 확인 문제
...
-- 정답: id는 직원 식별용 번호로 0~65,535 범위면 충분하기 때문에, 불필요하게 큰 자료형 대신 SMALLINT UNSIGNED를 사용하면 저장 공간과 인덱스 크기를 줄여 성능을 높일 수 있어 적합하다.
...
-- 정답: VARCHAR(50)은 가변 길이를 저장하는 자료형 길이가 정해지지 않은 값을 넣을 때 사용하면 메모리 효율이 좋음
-- CHAR(50)은 고정길이 문자열 최대 50글자 까지 저장하고 남은 공간은 공백으로 채움 메모리 효율이 안좋음 (주민등록번호나 문자열의 길이가 정해진 곳에 씀)
...
-- 정답: 성별을 M , F 둘 중 하나의 값만 저장할 수 있게 만듬 잘못된 입력을 예방
...
-- 정답: 둘다 근사값을 저장하는데 FLOAT는 소수점 7자리까지 나타내고 DOUBLE은 15자리 까지 나타냄 DOUBLE이 더 정확함
...
SELECT title , event_date
FROM  events
WHERE event_date BETWEEN '2023-10-01' AND '2023-12-31' ORDER BY event_date DESC;
