-- Active: 1758675368804@@127.0.0.1@3306@company
-- 1. 서브쿼리 확인 문제
...
-- 정답:
SELECT e.name AS 이름 , COUNT(ep.project_id) AS '프로젝트 개수'
FROM employees e
...
-- 정답: 
SELECT
	sub.department_id AS '부서 코드',
	d.name AS '부서명',
	sub.total_employee AS '직원 수'
FROM (
...
-- 정답: 
SELECT e.name , sub.max_amount 
FROM employees e
JOIN (
	SELECT MAX(amount) AS 'max_amount' 
	FROM salary_records
) sub ON sub.max_amount = e.salary;
...
-- 정답:
SELECT p.name , COUNT(employee_id)
FROM employee_projects ep
