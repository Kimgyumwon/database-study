-- Active: 1758675368804@@127.0.0.1@3306@bank
/*
	4. 데이터 집계하기
...
-- 어떤 행의 ㄱ밧을 보여줄지 MySQL이 결정할 수 없어서 에러를 뱉음
...
-- 위 쿼리를 IN(목록에 포함된 값 찾기) 연산자를 활용하여 나타내기
SELECT COUNT(*)
FROM transactions
WHERE msg IN('Amazon' ,'Coupang');
-- IN 의미: msg가 () 안에 포함이 되어 있다면 TRUE
-- IN 연산자를 사용하면 더 직관적이고 편리함
