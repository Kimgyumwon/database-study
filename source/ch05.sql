-- Active: 1758675368804@@127.0.0.1@3306@data_type
/*
	5. 다양한 자료형 활용하기
	5.1 자료형이란
*/
...
-- ① id는 기본키로 선언됐다. o
-- ② name은 최대 255자까지 저장할 수 있다. o
-- ③ price에 저장할 수 있는 최댓값은 9,999,999,999이다. x
-- ④ customer_name이 100자보다 짧으면, 고객명을 저장하고 남은 만큼의 메모리 공간이 절약된다. x
-- ⑤ created_at에는 날짜와 시간 값을 모두 저장할 수 있다. o
...
-- CONCAT_WS(구분자,문자열2,..): CONCAT과 비슷하지만, 첫 번째 인자로 '구분자'를 받아 각 문자열 사이에 자동으로 넣어줌
SELECT CONCAT_WS(' / ' ,name,price,quantity) AS order_details
FROM orders;

-- UPPER / LOWER : 문자열을 대문자 , 소문자로 변경
-- 예: 이메일 검색 등에서 대소문자 구분 없이 비교해야 할 때 유용
SELECT name,UPPER(name)
FROM orders;

-- LENGHT(): 문자열의 길이를 바이트 반위로 변환
-- CHAR_LENGHT(): 글자 수를 반환
SELECT name, CHARACTER_LENGTH(name) AS char_lenght , LENGTH(name) AS byte_lenght
FROM orders;
...
-- 4. 특정 범위 필터링하기
-- 1) BETWEEN 연산자
-- 두 값 사이에 속하는지 확인할 때 사용하는 연산자
SELECT *
FROM 테이블명
WHERE 컬럼명 BETWEEN 시작 AND 끝;
...
-- (참고) 상품 가격이 10000 ~ 20000원 사이가 아닌 주문은?
SELECT *
FROM orders
...
VALUES
	...
	(22, '나내산 고추장', 5387.75, 1, '2024-10-24 01:19:44');
