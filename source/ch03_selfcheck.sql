...
-- 3. 모든 버거의 100g당 가격을 다음과 같이 조회하세요.
  SELECT name AS '버거 이름', FLOOR(price/gram*100) AS '100g당 가격'
  FROM burgers;
-- ---------------------
-- 버거 이름  | 100g당 가격
-- ---------------------
-- 4. 100g당 가격이 2500원 미만인 버거를 다음과 같이 조회하세요.
  SELECT name , price AS 가격, gram AS '무게(g)' , FLOOR(price/gram*100) AS '100g당 가격'
  FROM burgers
  WHERE FLOOR(price/gram*100) < 2500;
-- --------------------------------------------
