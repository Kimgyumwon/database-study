-- Active: 1758675368804@@127.0.0.1@3306@mapdonalds
/*
  3.데이터 필터링하기
  3.1 데이터 필터링이란
*/

-- 많은 데이터 중에 원하는 데이터만 골라내는 작업
...
-- Quiz 5500원 보다 싸거나, 동시에 단백질량이 25g 보다 많은 버거
  SELECT *
  FROM burgers
...
-- 가독성과 이식성을 위해서 AND , OR , NOT을 사용하는 것이 좋다
...
SELECT *, FLOOR(price/gram*100) AS '100g 가격'
FROM burgers;
...
-- 4. 모든 학생의 평균은?
SELECT * , FLOOR((math + english + programming)/3) AS 평균
FROM students;

-- 5. 총점이 270 이상인 학생의 닉네임, 총점, 평균은?
SELECT nickname , math + english + programming AS 총점 , FLOOR((math + english + programming)/3) AS 평균
FROM students
