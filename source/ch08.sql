...
-- OFFSET = (현재 페이지 번호 - 1) * 페이지당 컨텐츠 보여줄 개수
...
-- 상품 유형별 집계하기
-- 상품(products) 테이블에서 상품 유형별 상품의 개수 , 최고 가격, 최저 가격
SELECT 
...
SELECT p.name AS 상품명 , SUM(count) AS 판매수량
FROM products p -- 1) products 테이블을 불러온다
JOIN order_details od ON p.id = od.product_id -- 2) order_details 테이블과 product_id 로 조인
JOIN orders o ON od.order_id = o.id -- 3) 순서 2의 결과에서 orders 테이블과 orders_id로 조인
	AND o.status = '배송 완료' -- 4) 순서 3의 결과에서 orders테이블에 status컬럼의 값이 '배송 완료' 것만 표시
GROUP BY p.id 
ORDER BY SUM(count) DESC , p.name
LIMIT 3;


SELECT u.nickname , p.name , count
FROM products p
JOIN order_details od ON p.id = od.product_id
JOIN orders o ON od.order_id = o.id
JOIN users u ON o.user_id = u.id
WHERE o.status = '배송 완료'
ORDER BY nickname , count DESC;
