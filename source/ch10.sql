-- Active: 1758675368804@@127.0.0.1@3306@data_modeling
/*
	10. 데이터 모델링
	10.1 데이터 모델링이란
*/
...
-- ① _____ㄴ_____: 비즈니스에서 중요 대상을 모형화한 것으로 데이터베이스로 구현했을 때 테이블이 됨
-- ② _____ㅁ_____: 엔티티의 실제 데이터로, 테이블에 저장된 행 단위의 데이터
-- ③ _____ㄱ_____: 엔티티의 특성이나 정보를 나타내는 항목으로 데이터베이스로 구현했을 때 칼럼이 됨
-- ④ _____ㄹ_____: 엔티티의 한 인스턴스가 다른 엔티티의 몇 개 인스턴스와 연결되는지를 나타내는 개념
-- ⑤ _____ㄷ_____: 엔티티 내에서 각 인스턴스를 고유하게 식별하는 속성 또는 속성의 조합
...
-- users 테이블의 email 과 products 테이블의 name을 고유키로 지정
-- 고유키는 UK로 표시
-- products 테이블의 name을 인덱스 처리
-- 상품명으로 검색 및 정렬할 때 성능 향상
...
-- ① _____ㄹ_____: 엔티티 간 관계가 필수인지 선택인지를 정의하는 개념
-- ② _____ㅁ_____: 컬럼이 NULL 값을 가질 수 없게 하는 제약 조건
-- ③ _____ㄴ_____: 컬럼에 입력되는 값이 특정 조건을 만족하도록 하는 제약 조건
-- ④ _____ㄷ_____: 컬럼에 값이 입력되지 않을 때 기본으로 저장된 값을 설정하는 제약 조건
-- ⑤ _____ㄱ_____: 데이터베이스에서 데이터의 검색 및 정렬 성능을 빠르게 해 주는 데이터 구조로 테이블의 특정 컬럼에 생성해 활용
...
-- ① _____ㅁ_____: 데이터의 중복을 최소화하고 데이터 무결성을 보장하기 위해 데이터를 구조화 하는 작업으로, 
--               잘못된 데이터 모델링을 사전에 검증하고 개선하는 것
-- ② _____ㄱ_____: 테이블의 컬럼을 더 이상 분해할 수 없도록 나누는 것
-- ③ _____ㄷ_____: 일반 속성이 기본키의 전체 속성에 의해 결정되는 것
-- ④ ______ㄹ____: 일반 속성이 기본키의 일부 속성에 의해 결정되는 것
-- ⑤ ______ㄴ____: 일반 속성이 다른 일반 속성에 의해 값이 결정되는 것
